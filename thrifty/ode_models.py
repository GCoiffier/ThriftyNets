...
class ConvODENet(nn.Module):
    ...
    adjoint : bool
        ...
        backpropagates directly through operations of ODE solver.
    """
    def __init__(self, device, input_shape, n_classes, n_filters, activ="tanh", tol=1e-7, adjoint=False):
        ...
        self.embed_in = nn.Conv2d(self.input_shape[0], self.n_filters, kernel_size=3, padding=1, bias=False)
        self.embed_out = nn.Conv2d(self.n_filters, 2, kernel_size=3, padding=1, bias=False)

        odefunc = ConvODEFunc(device, n_filters, activ, bn=True)
        self.odeblock = ODEBlock(device, odefunc, tol=tol, adjoint=adjoint)
        self.Loutput = nn.Linear(2*input_shape[1]*input_shape[2], self.n_classes)
        
        self.n_parameters = sum(p.numel() for p in self.parameters())

    def forward(self, x, return_features=False):
        #features = F.pad(x, (0, 0, 0, 0, 0, self.n_filters - self.input_shape[0]))
        features = self.embed_in(x)
        features = self.odeblock(features)
        features = self.embed_out(features)
